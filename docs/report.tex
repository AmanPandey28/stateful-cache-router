...
We conducted extensive benchmarking to validate the system's performance and correctness.

\subsection{Scalability Benchmark}
To test the router's throughput under high load, we simulated 100 workers handling 1,000 concurrent requests.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{latency_distribution.png}
    \caption{Router Latency Distribution (100 Workers, 1000 Requests)}
    \label{fig:latency}
\end{figure}

The results (Fig. \ref{fig:latency}) show a median latency of 28.42ms and a p99 latency of 63.80ms. This demonstrates that the router's "Sticky-Least-Loaded" policy with reverse index optimization scales efficiently, maintaining low overhead even with a large cluster size.

\subsection{Consistency \& Recovery}
We validated the anti-entropy sync mechanism by simulating a worker crash where a prefix is evicted locally without reporting.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{recovery_timeline.png}
    \caption{Stale Cache Recovery Timeline}
    \label{fig:recovery}
\end{figure}

As shown in Fig. \ref{fig:recovery}, the system initially routes requests to the stale worker (False Hits) but successfully recovers exactly at the 5-second sync interval. This empirically proves the eventual consistency guarantee.

\subsection{Cache Hit Rate}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{cache_hit_rate.png}
    \caption{Cache Hit Rate Warming Curve}
    \label{fig:hitrate}
\end{figure}

Fig. \ref{fig:hitrate} illustrates the system's ability to "warm up" the cache. Starting from a cold state (5\% hit rate), the hit rate rapidly increases to $\sim$85\% as the router learns the worker states and directs requests accordingly.
